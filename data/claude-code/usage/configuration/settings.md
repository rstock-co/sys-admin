...
| `deny`                         | Array of [permission rules](/en/iam#configuring-permissions) to deny tool use. Files matching deny rules are completely invisible to Claude Code. Replaces deprecated `ignorePatterns`. **Note:** Bash patterns are prefix matches and can be bypassed (see [Bash permission limitations](/en/iam#tool-specific-permission-rules)) | `[ "WebFetch", "Bash(curl:*)", "Read(./.env)", "Read(./secrets/**)" ]` |
